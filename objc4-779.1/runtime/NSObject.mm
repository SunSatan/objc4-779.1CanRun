...
/**
 * storeWeak函数，更新weak变量
 * @discussion HaveOld: true - 变量有值，需要被及时清理，当前值可能为 nil
 * @discussion HaveNew: true - 需要被分配的新值，当前值可能为 nil
 * @discussion CrashIfDeallocating: true - 说明 newObj 已经释放或者 newObj 不支持弱引用，该过程需要暂停，false - 用 nil 替代存储
 */
...
storeWeak(id *location, objc_object *newObj)
{
    ...
    // goto语句, haveOld 和 haveNew 由编译器决定传入的值, location 是 weak 指针, newObj 是 weak 指针将要指向的对象
...
/**
 在以前的版本中，当objct正在dealloc时，我们可能会过早的的清除*location区域。
 像NSPointerFunctions，会尝试预检查原始存储区域，并假设存储区域为
 零。但是由于我们过早的清除了*location区域，所以导致weak系统仍然会检查并清除*location。最终会导致objc_weak_error和崩溃。因此，当dealloc完成之前，我们不要去操作我们的*location。
 */
